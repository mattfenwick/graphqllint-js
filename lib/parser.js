...
const _plainChar = node("plain char",
	["char", not1(oneOf("\"\\"))]);
...
const _directiveName = node("directive name",
	["open", literal("@")],
	["name", cut("name", _name)]);
...
const namedType = node("named type",
	["type", name]);
...
const fragmentName = node("fragment name",
	["name", check((node) => {
		const name = node.first + node.rest.join('');
		return name !== "on";
	}, name)]);
...
const fragmentSpread = node("fragment spread",
	["splat", splat],
...
const typeCondition = node("type condition",
	["open", on],
	["type", namedType]);

const inlineFragment = node("inline fragment",
	["splat", splat],
...
selectionSet.parse = node("selection set",
	["open", oc],
...
const fragmentDefinition = node("fragment definition",
	["open", fragment],
