...
function plainChar(node) {
	return node.char;
}

function string(node) {
	return ast.String(node._start, node.body.map(build));
}

function alias(node) {
	return ast.Alias(node._start, build(node.name));
}

function gql_arguments(node) {
	return ast.Arguments(node._start, node.arguments.map(build));
}

function argument(node) {
	return ast.Argument(node._start, build(node.name), build(node.value));
}
